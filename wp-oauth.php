...
Class WPOA {
	...
	private $settings = array(
		...
		// any string
		'wpoa_musicalrainbows_api_enabled'      => 0,
		// 0, 1
		'wpoa_musicalrainbows_api_id'           => '',
		// any string
		'wpoa_musicalrainbows_api_secret'       => '',
	...
	*/
	...
	function wpoa_match_wordpress_user( $oauth_identity ) {
		...
		$query_string   = "SELECT $users_table.ID FROM $users_table WHERE $users_table.user_email = '" . $oauth_identity['email'] . "'";
	...
	function wpoa_login_user( $oauth_identity ) {
		...
		if ( $matched_user ) {
			...
			$this->wpoa_check_oauth_id($user_id, $oauth_identity['provider'], $oauth_identity['id']);
	...
	// assign oauth id to user if it wasn't assigned before
	function wpoa_check_oauth_id($user_id, $provider, $oauth_id) {
		$identities = get_user_meta($user_id, 'wpoa_identity');

		if( is_array( $identities ) && sizeof( $identities ) != 0 ) {
			$found = false;
			foreach( $identities as $key => $identity ) {
				if( strpos( $identity, $provider ) !== false ) {
					$found = true;
				}
			}
			...
			if( ! $found ) {
				...
				//TODO send mail account is linked
			}
		}
	}
	...
	function wpoa_end_logout() {
		...
		$this->wpoa_redirect($last_url);
	}

	//redirect to other page
	function wpoa_redirect($last_url) {
	...
	function wpoa_link_account( $user_id ) {
		if ( $_SESSION['WPOA']['USER_ID'] != '' ) {
			add_user_meta( $user_id, 'wpoa_identity',
				$_SESSION['WPOA']['PROVIDER'] . '|' . $_SESSION['WPOA']['USER_ID'] . '|' . time() );
		}
	}
	...
	function wpoa_unlink_account() {
		...
		$query_string   = $wpdb->prepare( "DELETE FROM $usermeta_table WHERE $usermeta_table.user_id = $user_id AND $usermeta_table.meta_key = 'wpoa_identity' AND $usermeta_table.umeta_id = %d",
			$wpoa_identity_row );
	...
	function wpoa_login_form( $atts ) {
		...
		$html = $this->wpoa_login_form_content( $a['design'], $a['icon_set'], $a['layout'], $a['button_prefix'],
			$a['align'], $a['show_login'], $a['show_logout'], $a['logged_out_title'], $a['logged_in_title'],
			$a['logging_in_title'], $a['logging_out_title'], $a['style'], $a['class'] );
	...
	function wpoa_login_form_designs_selector( $id = '', $master = false ) {
		...
		if ( $master == true ) {
			...
			foreach ( $designs_array as $key => $val ) {
				$html .= "<option value='" . $key . "' " . selected( get_option( $name ), $key,
						false ) . ">" . $key . "</option>";
			}
			$html .= "</select>";
		}
	...
	function wpoa_settings_page() {
		add_options_page( 'WP-OAuth Options', 'WP-OAuth', 'manage_options', 'WP-OAuth',
			array( $this, 'wpoa_settings_page_content' ) );
	}
